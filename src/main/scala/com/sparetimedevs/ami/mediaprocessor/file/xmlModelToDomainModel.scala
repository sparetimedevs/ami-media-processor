...
import com.sparetimedevs.ami.core.{AttributesType, BeatType, Beats, Duration, EditorialSequence, Measure, MusicComponent, NotImplementedMusicComponent, Note, NoteOption, NoteType, Part, Pitch, Rest, ScorePartwise, Step, Time, TimeOption, TimeSignature, Unpitched}
...
private def toDomainModel(partXml: musicxml.Part): Part = {
  val measures: Seq[Measure] = partXml.measure.map((measureXml: musicxml.Measure) => toDomainModel(measureXml))
  Part(id = partXml.id, measures = measures)
}

private def toDomainModel(measureXml: musicxml.Measure): Measure = {
  val musicData: Seq[MusicComponent] = measureXml.musicu45dataOption11.map((musicDataRecord: scalaxb.DataRecord[musicxml.Musicu45dataOption1]) => {
    ...
    musicDataOption match {
      case attributesTypeXml: musicxml.AttributesType                                                                              => toDomainModel(attributesTypeXml)
      ...
      case noteXml: musicxml.Note                                                                                                  => toDomainModel(noteXml)
...
private def toDomainModel(attributesTypeXml: musicxml.AttributesType): AttributesType = {
...
private def toDomainModel(noteXml: musicxml.Note): Note =
  toDomainModel(noteXml.typeValue)
    .map { noteType =>
      val noteOption: NoteOption = noteXml.noteoption.as[musicxml.NoteOption] match {
        ...
        case musicxml.NoteSequence5(fullu45noteSequence1, durationSequenceXml, tie) =>
          ...
          fullu45noteSequence1.fullu45noteoption1.as[musicxml.Fullu45noteOption1] match {
            case pitchXml: musicxml.Pitch                                    => toDomainModel(pitchXml, noteType, durationSequenceXml, isNotePartOfChord)
            case musicxml.Rest(displayu45stepu45octaveSequence1, attributes) => Rest(noteType, toDomainModel(durationSequenceXml))
            case musicxml.Unpitched(displayu45stepu45octaveSequence1)        => Unpitched(noteType, toDomainModel(durationSequenceXml), isNotePartOfChord = isNotePartOfChord)
          }
      }
...
private def toDomainModel(noteTypeValueXml: Option[musicxml.Noteu45type]): Option[NoteType] =
